...
const LoginModal: React.FC = () => {
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            addToast(error.message || 'Erro ao realizar login com o Google.', 'error');
        } finally {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
            <div 
                className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-8 border border-slate-200 dark:border-slate-800 shadow-2xl relative overflow-hidden"
            ...
            >
                ...
                <div className="text-center mb-8">
                    <div className="w-16 h-16 bg-slate-900 dark:bg-slate-800 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-black/5">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
                        Acesse a plataforma com sua conta corporativa Google Workspace ou e-mail Google para sincronizar suas permissões e visualização.
                    </p>
                </div>

                {/* Domain Restriction Notice if active */}
                {workspaceSettings.domainRestrictionEnabled && workspaceSettings.allowedDomain && (
                    <div className="mb-6 p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 flex items-start gap-3 text-amber-800 dark:text-amber-300">
                ...
                <button
                    ...
                    className="w-full py-4 px-6 rounded-2xl bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-100 dark:text-slate-900 text-white font-bold text-sm flex items-center justify-center gap-3 transition-all shadow-lg hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50"
                >
                    {isLoggingIn ? (
                        <>
                ...
                <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 text-center">
                    <p className="text-[11px] text-slate-400 font-medium">
                        Sua primeira conta autenticada se torna o Administrador inicial.
                    </p>
                </div>
            </div>
        </div>
