...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
    ...
    return (
        <div className="flex flex-col h-full">
            {/* Header Area */}
            <div className="mb-10">
                <h1 className="text-3xl font-bold text-brand-dark dark:text-white mb-2 transition-colors">{t('dashboard.title')}</h1>
                <p className="text-slate-500 dark:text-slate-400 transition-colors">{t('dashboard.subtitle')}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {apps.map((app) => (
                    <button
                        ...
                        className="group flex flex-col p-6 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-glow hover:border-brand-pink/50 dark:hover:border-brand-pink/50 transition-all duration-300 text-left relative overflow-hidden"
                    >
                        <div className="flex items-start justify-between w-full mb-4">
                            <div className="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-700 dark:text-slate-300 group-hover:bg-brand-pink group-hover:text-white transition-colors duration-300">
                                {app.icon}
                            </div>
                            <div className="opacity-0 group-hover:opacity-100 transition-opacity -mr-2 -mt-2">
                                <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-full p-1.5 inline-flex">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
                                </span>
                            </div>
                        </div>
                        
                        <h3 className="font-bold text-brand-dark dark:text-white text-lg mb-1 group-hover:text-brand-pink transition-colors">{app.label}</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">{app.desc}</p>
                    </button>
                ))}
            </div>

            {/* Empty State / Bottom filler */}
            <div className="mt-auto pt-10 border-t border-slate-200/60 dark:border-slate-800/60">
                <div className="flex items-center gap-4 text-slate-400 dark:text-slate-600 text-sm">
